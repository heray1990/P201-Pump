...
en_result_t Flash_Manager_Update(void)
{
    ...
    boolean_t bIsTailOfSector = FALSE;
    ...
    if(FALSE == bIsTailOfSector)
    {
        // 该Sector未写满, 则新的数据写到该Sector的下一个Partition中
        stcFlashManager.u32DataStoredHeadAddr += FLASH_MANAGER_DATA_LEN;
    }
